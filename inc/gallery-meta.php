...
function gallery_meta_callback($post) {
	...
	$ids = get_post_meta($post->ID, 'dm_gallery_id', true);
	?>
				...
				<ul id="gallery-metabox-list" class="gallery_metabox_list">
					<?php if ($ids) : foreach ($ids as $key => $value) : $image = wp_get_attachment_image_src($value); ?>
						...
						<li class="image_holder">
							<input type="hidden" name="dm_gallery_id[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr($value); ?>">
...
function gallery_meta_save($post_id) {
	...
	if(isset($_POST['dm_gallery_id'])) {
		update_post_meta($post_id, 'dm_gallery_id', $_POST['dm_gallery_id']);
	} else {
		delete_post_meta($post_id, 'dm_gallery_id');
	}
}
